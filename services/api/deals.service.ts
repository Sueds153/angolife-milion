...
export const DealsService = {
  getDeals: async (isAdmin: boolean = false): Promise<ProductDeal[]> => {
    ...
    return data.map((d: any) => ({
      ...
      price: d.discount_price,  // alias usado em alguns componentes
      location: d.location,
      description: d.description,
      imagePlaceholder: d.image_placeholder,
      imageUrl: d.image_url || d.image_placeholder,  // fallback para registos antigos
      url: d.url,
      category: d.category,
  ...
  getPendingDeals: async (): Promise<ProductDeal[]> => {
    ...
    return data.map((d: any) => ({
      ...
      description: d.description,
      imagePlaceholder: d.image_placeholder,
      imageUrl: d.image_url || d.image_placeholder,
      url: d.url,
      category: d.category,
      status: "pending",
      submittedBy: d.submitted_by,
  ...
  submitDeal: async (
    deal: Omit<ProductDeal, "id" | "createdAt">,
  ): Promise<void> => {
    const { error } = await supabase.from("product_deals").insert([
      {
        title: deal.title,
        store: deal.store,
        store_number: deal.storeNumber || null,
        original_price: deal.originalPrice,
        discount_price: deal.discountPrice,
        location: deal.location,
        description: deal.description,
        image_placeholder: deal.imagePlaceholder || deal.imageUrl || '',
        image_url: deal.imageUrl || deal.imagePlaceholder || '',
