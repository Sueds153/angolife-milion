...
// Helper: formata descrições vindas do scraping que chegam como bloco de texto sem quebras
                ...
                <div onClick={() => handleOpenDetails(job)} className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] shadow-sm border border-orange-500/10 hover:border-orange-500/50 transition-all cursor-pointer group flex flex-col h-full hover:shadow-xl hover:-translate-y-1 relative group overflow-hidden">
                  ...
                  {(job.applicationCount || 0) > 0 && (
                    <div className="flex items-center gap-2 mb-6">
                      ...
                      <span className="text-[10px] font-black text-orange-500 uppercase tracking-tighter italic"><span className="animate-pulse inline-block">🔥</span> {job.applicationCount} pessoas candidataram-se</span>
                    </div>
                  )}
                  ...
                  <div className="pt-5 border-t border-orange-500/10 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`p-2.5 rounded-xl transition-all ${user?.savedJobs?.includes(job.id) ? 'bg-rose-500/10 text-rose-500 opacity-100' : 'bg-slate-100 dark:bg-white/5 text-slate-400 hover:text-rose-500 hover:bg-rose-500/10 opacity-100 lg:opacity-0 group-hover:opacity-100'}`}
                      >
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/90 backdrop-blur-md p-0 md:p-4 animate-fade-in overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 w-full max-w-4xl h-full md:h-auto md:rounded-[3rem] shadow-2xl border border-orange-500/20 relative overflow-hidden flex flex-col">
                ...
                <div className="lg:col-span-2 space-y-10">
                  <section>
                    ...
                    <div className="text-slate-600 dark:text-slate-300 text-sm md:text-base leading-relaxed font-medium space-y-3">
                      {formatDescription(selectedJob.description).map((paragraph, i) => (
                        <p key={i}>{paragraph}</p>
                      ))}
                    </div>
                  </section>
                  ...
                  {selectedJob.requirements && selectedJob.requirements.length > 0 && (
                    <section>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {(selectedJob.requirements.length === 1 && selectedJob.requirements[0].length > 50
                          ? formatDescription(selectedJob.requirements[0])
            ...
            {/* Mobile Sticky CTA */}
            <div className="md:hidden fixed bottom-0 left-0 w-full p-4 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-t gold-border-t-subtle z-[40] flex items-center gap-3">
              <button
                onClick={() => handleApplyClick(selectedJob)}
