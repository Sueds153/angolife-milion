...
export const NewsPage: React.FC<NewsPageProps> = ({ onRequestReward }) => {
  ...
  useEffect(() => {
    // Gestor de fallback para manter sincronização offline perfeitamente
    if (rawNews.length > 0) {
      localStorage.setItem('news_cache', JSON.stringify(rawNews.slice(0, 10)));
      setNews(rawNews);
      setLoading(false);
    } else if (isOffline || !rawNews.length) {
  ...
  }, [rawNews, isOffline, queryLoading]);
  ...
  const handleArticleClick = (article: NewsArticle) => {
    ...
    // Every X clicks: show a short 15s Rewarded Video to unlock the body
    if (newCount % AD_CONFIG.REWARD_AD_THRESHOLD === 0) {
