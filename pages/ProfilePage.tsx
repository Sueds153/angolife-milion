...
export const ProfilePage: React.FC = () => {
  ...
  // ── REGRA DE OURO DO REACT: TODOS OS HOOKS ANTES DE QUALQUER RETURN ──
  ...
  // Sincroniza os campos de edição sempre que o user muda no store (ex: após login)
  useEffect(() => {
    if (user) {
  ...
  useEffect(() => {
    if (!user?.savedJobs || user.savedJobs.length === 0) return;
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-white/5">
             ...
             <div className="w-full h-2 bg-slate-50 dark:bg-white/5 rounded-full overflow-hidden">
                 <div 
                    ...
                    className="h-full bg-gradient-to-r from-orange-600 to-amber-400 rounded-full transition-all duration-1000 shadow-[0_0_15px_rgba(249,115,22,0.3)]"
                 />
             </div>
             {profileProgress < 100 && (
