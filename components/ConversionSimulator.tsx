...
export const ConversionSimulator: React.FC<ConversionSimulatorProps> = ({
  ...
  const calculateConversion = () => {
    ...
    const foreignCurrency =
      baseCurrency === "AOA" ? targetCurrency : baseCurrency;
    ...
    if (!rate) return "0.00";
    const result =
      baseCurrency === "AOA" ? baseAmount / rate : baseAmount * rate;
      ...
      <div className={!isAuthenticated ? "blur-md pointer-events-none" : ""}>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative" onClick={(e) => e.stopPropagation()}>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setOpenDropdown(openDropdown === "base" ? null : "base");
                  }}
                ...
                >
                  <img
                    src={`https://flagcdn.com/${baseCurrency === "AOA" ? "ao" : baseCurrency === "USD" ? "us" : "eu"}.svg`}
                ...
                {openDropdown === "base" && (
                  <div className="absolute right-0 mt-2 w-40 bg-white dark:bg-slate-800 border border-orange-500/20 rounded-2xl shadow-2xl z-[100] overflow-hidden animate-fade-in">
                    {(["AOA", "USD", "EUR"] as const).map((cur) => (
                      ...
                      >
                        <img
                          src={`https://flagcdn.com/${cur === "AOA" ? "ao" : cur === "USD" ? "us" : "eu"}.svg`}
                          className="w-4 h-4 rounded-full"
                          alt={cur}
                        />
                        <span
                          className={`font-black text-xs uppercase tracking-widest ${baseCurrency === cur ? "text-orange-500" : "text-slate-500 dark:text-slate-400"}`}
                        >
                          {cur}
                        </span>
                      </button>
          ...
          <div className="bg-white dark:bg-black p-6 md:p-8 rounded-[2rem] border border-orange-500/20 shadow-sm">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative" onClick={(e) => e.stopPropagation()}>
                ...
                >
                  <img
                    src={`https://flagcdn.com/${targetCurrency === "AOA" ? "ao" : targetCurrency === "USD" ? "us" : "eu"}.svg`}
                    className="w-6 h-6 rounded-full"
                  ...
                  <div className="absolute right-0 mt-2 w-40 bg-white dark:bg-slate-800 border border-orange-500/20 rounded-2xl shadow-2xl z-[100] overflow-hidden animate-fade-in">
                    {(["AOA", "USD", "EUR"] as const).map((cur) => (
                      ...
                      >
                        ...
                        />
                        <span
                          className={`font-black text-xs uppercase tracking-widest ${targetCurrency === cur ? "text-orange-500" : "text-slate-500 dark:text-slate-400"}`}
                        >
                          {cur}
                        </span>
                      </button>
        ...
        <div className="mt-10 pt-8 border-t border-orange-500/20 flex justify-between items-center px-4">
          ...
          <span className="text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase tracking-widest">
            1 USD ={" "}
            {(isOfficial
              ? rates.find((r) => r.currency === "USD")?.formalSell
              : rates.find((r) => r.currency === "USD")?.informalSell
