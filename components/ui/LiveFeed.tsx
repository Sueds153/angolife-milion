...
import { Activity, Flame, CheckCircle } from 'lucide-react';
          ...
          <p className="text-xs font-bold text-slate-800 dark:text-white leading-tight">
            {currentFeed.type === 'buy' ? (
              <span className="flex items-center gap-1.5">
                ...
                <span><span className="text-brand-gold">{currentFeed.name}</span> acabou de receber <span className="text-green-500">{currentFeed.amount} {currentFeed.currency}</span> na <span className="font-black underline">{currentFeed.wallet}</span>.</span>
              </span>
            ) : (
              <span className="flex items-center gap-1.5">
                <CheckCircle size={14} className="text-green-500 inline" />
                <span><span className="text-brand-gold">{currentFeed.name}</span> acabou de vender <span className="text-brand-gold">${currentFeed.amount}</span> e recebeu os Kwanzas no <span className="font-black underline">{currentFeed.bank}</span>.</span>
              </span>
            )}
          </p>
        </div>
