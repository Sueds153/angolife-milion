        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {pendingDeals.map((deal) => (
            <div key={deal.id} className="bg-white dark:bg-slate-900 border border-orange-500/20 rounded-[2rem] overflow-hidden shadow-xl shadow-amber-500/5 group relative">
              ...
              {(deal.imageUrl || deal.imagePlaceholder) && (
                <div className="h-36 overflow-hidden bg-slate-100 dark:bg-white/5 relative">
                  <img
                    src={deal.imageUrl || deal.imagePlaceholder}
                    alt={deal.title}
                  ...
                  />
                  {deal.is_admin && (
                    <span className="absolute top-2 left-2 bg-brand-gold text-slate-950 text-[8px] font-black px-2 py-1 rounded-full uppercase tracking-widest shadow">
                      Admin
                    </span>
                  )}
                </div>
