...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onLogin, onOpenLegal, initialMode = 'login' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        const { error } = await AuthService.signUp(cleanEmail, password, fullName, invitedBy);
        if (error) {
          if (error.message.toLowerCase().includes('rate limit')) {
            setErrorMsg('Limite de tentativas excedido. Por favor, aguarde uns minutos.');
          } else {
        ...
        const { error } = await AuthService.signIn(cleanEmail, password);
        if (error) {
          // Mostrar erro específico do Supabase (ex: e-mail não confirmado)
          if (error.message.includes('Email not confirmed')) {
            setErrorMsg('O seu e-mail ainda não foi confirmado. Verifique a sua caixa de entrada.');
          } else if (error.message.includes('Invalid login credentials')) {
            setErrorMsg('E-mail ou palavra-passe incorretos. Verifique os dados.');
          } else {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-2 md:p-4">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl flex flex-col relative border border-orange-500/30 max-h-[min(90vh,700px)] overflow-hidden">
        <button
          onClick={onClose}
          title="Fechar"
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors z-[101] bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-full p-1 shadow-sm"
        >
          <X size={24} />
        </button>
        ...
        <div className="bg-slate-50 dark:bg-slate-800 p-4 text-center text-sm border-t gold-border-t-subtle shrink-0">
